...
def full_conn():
    ...
    # 2,建立数据的占位符，批次输入， x [None, 784]  y_true [None, 10]
    with tf.variable_scope("data"):
        x = tf.placeholder(tf.float32, [None, 784])

        y_true = tf.placeholder(tf.int32, [None, 10])

    # 3,建立一个全连接层的神经网络  w [784, 10]  b [10]
...
def model():
    ...
    # 2,一层卷积 卷积：5*5*1, 32个, strides=1 激活：tf.nn.relu 池化
    ...
    # 3，二层卷积：5*5*32， 64个filter，strides=1 激活：tf.nn.relu 池化：
    with tf.variable_scope("conv2"):
        # 随机初始化权重和偏置, 权重：[5, 5, 32, 64]----->[None, 7, 7, 64]
